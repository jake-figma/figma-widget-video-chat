(() => {
  ...
  var USER_ID = Math.round(Math.random() * 1e14).toString().padStart(15, "0");
  var WebRTC = class {
    constructor(api) {
      this.api = api;
      this.connections = {};
  ...
  var API = class {
    ...
    receiveMessageFromParent({ type, data }) {
      if (type === "pong") {
        if (!this.initialized) {
  ...
  var dom = new Dom(document.querySelector("main"));
})();
